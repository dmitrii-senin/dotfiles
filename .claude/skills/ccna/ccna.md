...
**Plan start date:** Monday **2026-05-04** (this is the source of truth for the `/ccna schedule` mode — change it here if the start shifts). *Originally 2026-04-27; shifted +7 days on 2026-05-07 to absorb a holiday week (2026-05-11 to 2026-05-17).*
...
- **Milestone:** `/ccna quiz domain 1 25` — target ≥ 70%
...
- **Milestone:** `/ccna quiz domain 1,2 30` — target ≥ 72%
...
- **Major milestone:** `/ccna quiz mock 50` — target ≥ 70%
- Use `/ccna cli-roleplay ospf-troubleshoot` (Claude pretends to be a broken router; you fix it via CLI)
...
- **Milestone:** `/ccna quiz domain 4,5 30` — target ≥ 72%
...
- Run `/ccna quiz weak-areas` (Claude pulls from memory ledger of low-scoring topics — see Claude-assisted prep section)
...
- Review every wrong answer — write a one-line "why" for each (feed into `/ccna flash add` so the missed concepts get spaced-repeated)

**Weeks 27-28 — Confidence consolidation (no new material)**
- Daily flashcards (`/ccna flash review`)
- Re-do weakest-area quizzes from `/ccna quiz weak-areas`
...
The 6-month timeline + your absence of a question-bank-on-demand makes Claude an ideal study partner. Build **one custom skill** at `~/.claude/skills/ccna/` (source in `~/x/dotfiles/.claude/skills/ccna/`) that exposes several modes, plus a small persistent ledger for tracking weak topics across sessions.

### Skill: `ccna` (single skill, multiple modes)

| Invocation | What it does |
|---|---|
| `/ccna quiz <domain\|topic\|weak-areas\|mock> [N]` | Generates N exam-style questions (multiple choice + multi-select), grades, explains every answer with the *exam-relevant* reasoning, logs missed topics to the weak-areas ledger |
| `/ccna flash <add\|review>` | Lightweight spaced-repetition flashcard system stored as JSON. `add` ingests a fact you missed; `review` surfaces due cards (Leitner box scheduling — simpler than full SM-2) |
| `/ccna subnet [N] [--ipv6]` | Generates N subnetting problems (default 10), times you, grades. IPv6 mode for week 14+ |
...
Three small files maintained by the skill (under `~/.claude/skills/ccna/data/`, gitignored):

- `data/weak-areas.json` — `{topic: {misses, last_seen, last_score}}`. Drives `/ccna quiz weak-areas`.
...
- `data/command-journal.md` — plain-text IOS command journal for `/ccna journal`.
...
6. Save a memory note: "use `/ccna` skill before answering CCNA questions; cross-reference weak-areas.json"
...
1. **Daily subnetting drill (10 min)** — `/ccna subnet 10`. Non-negotiable from week 1 onward.
...
3. **Daily flashcards (10-15 min)** — `/ccna flash review`. Imports the **Neil Anderson Udemy Anki deck** (downloaded from Sec 2 of the course) into your local ledger; appended with cards you add as you study, plus Jeremy's free Anki deck as supplement. Critical against a 6-month forgetting curve.
4. **Weekly mini-quiz (Sunday, 15 min)** — `/ccna quiz weak-areas 15`. Targets exactly the topics you've been missing.
...
- Can subnet any /prefix in < 30 seconds without paper (verified by `/ccna subnet 20 --timed`)
...
- Can complete `/ccna cli-roleplay ospf-troubleshoot` and `/ccna cli-roleplay vlan-build` end-to-end without referring to notes
- Can explain (out loud, to a rubber duck — or via `/ccna tutor`) the difference between: trunk vs access port, RSTP port roles/states, OSPF neighbor states, standard vs extended ACL placement rules, controller-based vs traditional networking
...
- **Udemy practice tests reserved for weeks 21 + 24 + 26** — used as full-length, exam-condition mock exams. Earlier-phase domain quizzes are generated by Claude (`/ccna quiz domain N`), since the Udemy course doesn't filter cleanly by topic.
...
- **Plan source of truth:** this file is now the canonical copy at `~/x/dotfiles/.claude/skills/ccna/ccna.md` (the original at `~/ccna.md` can be deleted or symlinked: `ln -sf ~/x/dotfiles/.claude/skills/ccna/ccna.md ~/ccna.md`).
