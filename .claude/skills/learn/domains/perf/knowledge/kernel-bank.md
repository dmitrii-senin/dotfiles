...
**Key concepts:** taskset, sched_setaffinity, pthread_setaffinity_np, cpuset.cpus, isolcpus, irqaffinity
**Tip:** `isolcpus` alone is not sufficient: check `/proc/interrupts` after isolation to verify no IRQs are still targeting your isolated cores, and use `tuna --cpus=3 --isolate` to also move kernel threads away. In containers, `isolcpus` is invisible — pin via the `cpuset.cpus` cgroup instead, and *verify* the mask actually took with `taskset -pc <pid>` or `grep Cpus_allowed_list /proc/<pid>/status` rather than trusting the launch flags.
...
### NIC tuning with ethtool: rings, coalescing, offloads
`ethtool` is the primary interface for tuning the NIC itself, and three knobs dominate latency and drop behavior. **Ring (descriptor) size** (`ethtool -g` to view, `-G rx 4096` to set) sets how many packets the NIC can DMA into host memory before software drains them; rings that are too small drop micro-bursts (visible as `rx_no_buffer_count`), while oversized rings add a few microseconds of buffering. **Interrupt coalescing** (`-c`/`-C rx-usecs N rx-frames N`, `adaptive-rx off`) batches packets per interrupt — great for throughput/CPU, but each microsecond of `rx-usecs` is added latency, so a feed handler wants it low or zero (paired with busy-polling). **Offloads** (`-k` to view, `-K` to set: GRO, LRO, GSO, TSO, checksum) trade per-packet CPU for aggregation; **GRO/LRO coalesce received segments and can hold packets tens of microseconds**, which is poison on a feed where every datagram is a discrete message — turn them off.
**Key concepts:** ring descriptors (-g/-G), rx_no_buffer_count, coalescing (-C rx-usecs/rx-frames, adaptive-rx), offloads (-k/-K), GRO/LRO hold-back
**Tip:** Raise the RX ring before reaching for a bigger socket buffer — the ring absorbs the NIC-to-kernel burst at market open, while the socket buffer only helps once packets are already past the ring. Confirm the change took effect with `ethtool -g eth0` (preset maximums cap what you can request).
**Tool anchor:** `ethtool -g eth0; ethtool -c eth0; ethtool -k eth0 | grep -E 'generic-receive|large-receive'`
**Drill:** At market open your handler logs gaps and `ethtool -S eth0` shows `rx_no_buffer_count` climbing. Decide among enlarging the RX ring, lowering coalescing, and growing `SO_RCVBUF`: explain what each fixes, in what order to try them, and quantify the latency/CPU cost of disabling coalescing entirely.
**Tags:** ethtool, ring-buffer, coalescing, offloads, GRO, packet-drops, reliability

### NIC multiqueue: RSS, RPS/RFS, and IRQ steering
Modern NICs spread received packets across multiple RX queues so multiple cores share `NET_RX` softirq work. **RSS** hashes the 4-tuple in hardware to pick a queue (`ethtool -l`/`-L combined N` for queue count, `-x` to view the hash + indirection table, `-X` to set them); each queue has its own MSI-X interrupt, so steering is ultimately about which core each queue's IRQ lands on (`/proc/irq/<N>/smp_affinity`). Because hash-based RSS can't *guarantee* which core a given multicast flow hits, **ntuple flow steering** (`ethtool -N eth0 flow-type udp4 dst-port <p> action <q>`) directs a specific feed to a chosen queue deterministically. **RPS/RFS** are the software fallback (`/sys/class/net/*/queues/rx-*/rps_cpus`) when the NIC lacks enough queues. For a market-data box, the goal is to keep the feed's `NET_RX` softirq **off** the isolated handler core entirely.
**Key concepts:** RSS (-l/-L, -x/-X indirection table), ntuple steering (-N), RPS/RFS, MSI-X per-queue IRQ, smp_affinity
**Tip:** Steer the feed's RX queue with ntuple, then pin *that queue's* IRQ to a housekeeping core; verify in `/proc/interrupts` that the queue's interrupt count grows only on the housekeeping CPU and in `/proc/softirqs` that `NET_RX` stays flat on the isolated core. See also "Interrupts and softirqs" and "CPU affinity and isolcpus".
**Tool anchor:** `ethtool -x eth0; grep -E "$(basename eth0)|eth0" /proc/interrupts`
**Drill:** `NET_RX` softirqs are landing on isolated core 3 where your decoder runs. Using `ethtool -x` to read the indirection table and `ethtool -N` to steer the feed flow, move its RX queue's IRQ to a housekeeping core, and prove via `/proc/interrupts` + `/proc/softirqs` that core 3 no longer services network softirqs.
**Tags:** ethtool, RSS, ntuple, RPS, RFS, IRQ-affinity, isolation, determinism
...
### Hardware timestamping, PTP, and drop localization
For market data you need to measure *wire* latency and localize packet loss precisely, and both depend on the NIC. `ethtool -T eth0` reports the device's timestamping capabilities and its PTP Hardware Clock (PHC) index; `SO_TIMESTAMPING` with `SOF_TIMESTAMPING_RX_HARDWARE` then attaches a NIC-generated RX timestamp to each packet (read via `recvmsg` control messages), which is immune to the scheduling and syscall jitter that pollutes software timestamps. The PHC itself (`/dev/ptp0`) is disciplined to grandmaster time by `ptp4l`, and `phc2sys` copies PHC time into the system clock — giving cross-host, sub-microsecond comparable timestamps. **Drop localization** is the other half: a sequence-number gap with *no* local counter movement points upstream (wire/switch/A-feed-vs-B-feed arbitration); `ethtool -S` NIC counters (`rx_missed_errors`, `rx_no_buffer_count`, `rx_fifo_errors`) implicate the NIC ring; and `/proc/net/udp` `drops` / `netstat -su` / `/proc/net/softnet_stat` implicate the kernel/socket. Localize *before* tuning so you fix the right layer.
**Key concepts:** ethtool -T, SO_TIMESTAMPING (RX_HARDWARE), PHC /dev/ptp0, ptp4l/phc2sys, rx_missed_errors vs rx_no_buffer_count, drop localization
**Tip:** Hardware RX timestamps are taken at the NIC before the kernel touches the packet, so `(app_recv_time − hw_rx_time)` isolates host-stack latency from wire latency; if that delta is small but you still see gaps, the loss is on the wire, not your box. Confirm HW timestamping is actually available with `ethtool -T eth0` (many virtual NICs report none).
**Tool anchor:** `ethtool -T eth0; ethtool -S eth0 | grep -E 'rx_(missed|no_buf|fifo|drop)'`
**Drill:** You see gaps in CME MDP 3.0 sequence numbers. Build a localization checklist: which `ethtool -S` counters distinguish a NIC-ring drop from a wire loss, which `/proc/net` counters reveal a kernel/socket drop, and how a HW-vs-software RX timestamp delta confirms whether the host stack or the wire is responsible. State the fix for each case.
**Tags:** ethtool, hardware-timestamping, PTP, ptp4l, drop-localization, reliability
