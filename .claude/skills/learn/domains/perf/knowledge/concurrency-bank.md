...
**Tool anchor:** `bpftrace -e 'kprobe:_do_fork { @thread_creates[ustack(5)] = count(); }'` to detect unexpected thread creation in hot paths
...
**Tip:** `__thread` (C/GNU extension) forces initial-exec model and is faster than `thread_local` (which defaults to general-dynamic for shared libraries); if you control the link model, prefer `__thread` for hot-path TLS variables.
...
**Tip:** Userspace RCU's `rcu_read_lock()/rcu_read_unlock()` in `liburcu`'s QSBR flavor are literally no-ops (just compiler barriers) if you call `rcu_quiescent_state()` periodically in your event loop; this is perfect for market data handlers with a natural per-message quiescent point.
